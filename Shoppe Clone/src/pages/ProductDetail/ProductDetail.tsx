...
import purchaseAPI, { AddToCartType } from "src/apis/purchase.api";
...
export default function ProductDetail() {
  ...
  const handleZoom = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    //Cách 1: đã xử lý bubble event bằng pointer-events-none ở className css img
    // const { offsetX, offsetY } = event.nativeEvent
    //Cách 2 : bất chấp không cần xử lý bubble event dùng khi không css được element
    ...
    <div className='bg-gray-200 py-6'>
            ...
            <div className='col-span-7'>
              ...
              <div className='mt-8 flex items-center bg-gray-50 px-5 py-4'>
                ...
                <div className='ml-4 rounded-sm bg-orange px-1 py-[2px] text-xs font-semibold uppercase text-white'>
                  {calculateRateSale(product.price_before_discount, product.price)} {t('product detail.off')}
                </div>
              </div>
              <div className='mt-8 flex items-center'>
                <div className='capitalize text-gray-500'>{t('product detail.quantity')}</div>
                <QuantityController max={product.quantity} value={buyCount} onType={handleChangeBuyCount}
                  onIncrease={handleChangeBuyCount} onDecrease={handleChangeBuyCount} />
                <div className='ml-6 text-sm text-gray-500'>{product.quantity} {t('product detail.available')}</div>
              </div>
              <div className='mt-8 flex items-center'>
                <button className='flex h-12 items-center justify-center rounded-sm border border-orange bg-orange/10 px-5 capitalize text-orange shadow-sm hover:bg-orange/5' onClick={addToCart}>
                  <svg
                ...
                <button className='flex ml-4 h-12 min-w-[5rem] items-center justify-center rounded-sm bg-orange px-5 capitalize text-white shadow-sm outline-none hover:bg-orange/90' onClick={buyNow}>
      ...
      <div className='container ml-12'>
        <div className='mt-8 bg-white p-4 shadow'>
          <div className='rounded bg-gray-50 p-4 text-lg capitalize text-slate-700'>{t('product detail.product description')}</div>
      ...
      <div className="container ml-12">
        <div className="uppercase text-gray-400 mt-8">{t('product detail.also like')}</div>
        {productData && (
          <div className="mt-2">
